...
> [!WARNING]
> ### 🔒 第二铁律：敏感信息主动预警与零硬编码 (Security Gate Rule)
> **凡是涉及密码、API Token、秘钥、私钥、个人凭据等敏感信息时，Agent 必须强制先停下并向用户发出安全风险提醒并确认，绝对禁止直接将敏感信息硬编码写入代码或提交至 Git 仓库！**  
> 敏感信息应一律通过命令行参数、临时环境变量（如 `TG_TOKEN`）或仅存在于 VPS 本地的 600 权限配置文件中传递。
